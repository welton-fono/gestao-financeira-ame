...
with aba_financeiro:
    ...
    if not df.empty:
        ...
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            df_view.to_excel(writer, index=False, sheet_name='AME')
        st.download_button("📥 BAIXAR RELATÓRIO EXCEL", buffer, f"AME_{datetime.now().strftime('%d_%m_%Y')}.xlsx", use_container_width=True)

        st.divider()
        
        for i, row in df_view.iterrows():
            tem_nf = row.get('url_nf') != "" and row.get('url_nf') is not None
            status_cor = "🟢" if tem_nf else "🔴"
            
            with st.expander(f"{status_cor} {row['empresa']} | {row['funcionario']} | R$ {row['valor']:.2f}"):
                col_a, col_b = st.columns([2, 1])
                with col_a:
                    st.write(f"**Data Exame:** {row.get('data_formatada')} às {row.get('hora')}")
